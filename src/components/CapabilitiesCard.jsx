...
const ServiceCard = ({ icon: Icon, number, title, description, tags, isActive }) => {
    return (
        <div
            className="w-[320px] md:w-[380px] shrink-0 h-full transition-all duration-600"
            style={{
            ...
            <div className="relative h-[440px] bg-[#0a0a0a] border border-white/10 overflow-hidden group transition-all duration-500 hover:border-[#2E5BFF] shadow-2xl">
                ...
                <div className="relative p-8 h-full flex flex-col">
                    ...
                    <div className="flex items-start justify-between mb-8">
                        <div className="w-12 h-12 bg-[#2E5BFF] flex items-center justify-center shadow-[0_0_20px_rgba(46,91,255,0.4)] group-hover:shadow-[0_0_30px_rgba(46,91,255,0.6)] transition-shadow duration-300">
                            <Icon className="w-6 h-6 text-[#FCFCFC]" strokeWidth={2} />
                        </div>
                        <span className="text-white/10 text-3xl font-black tracking-tighter">0{number}</span>
                    </div>
                ...
                {/* Bottom Shine Effect on Hover */}
                <div className="absolute -bottom-10 -left-10 w-40 h-40 bg-[#2E5BFF]/20 rounded-full blur-3xl opacity-0 group-hover:opacity-100 transition-opacity duration-700" />
            </div>

            {/* Bottom Accent Bar */}
            <div className={`h-1 bg-[#2E5BFF] transition-all duration-300 ${isActive ? 'w-full' : 'w-0'
                }`} />
        </div>
    );
};
...
export default function CapabilitiesCard() {
    ...
    useEffect(() => {
        const handleResize = () => {
            if (window.innerWidth < 768) setCardWidth(320);
        ...
        <div className="min-h-screen relative z-10 w-full bg-[#050505] flex items-center justify-center py-20 font-sans overflow-hidden">
            ...
            <div className="w-full relative z-10">
                ...
                <div className="absolute left-1/2 -translate-x-1/2 top-1/2 w-full max-w-[1200px] flex justify-between px-10 z-50 pointer-events-none">
                    <button
                        onClick={prevSlide}
                        className="w-12 h-12 bg-[#2E5BFF] text-[#FCFCFC] flex items-center justify-center hover:scale-110 active:scale-95 transition-all shadow-xl pointer-events-auto border border-[#2E5BFF] hover:shadow-[0_0_20px_rgba(46,91,255,0.6)]"
                    >
                ...
                <div className="relative overflow-visible">
                    <div
                        className="flex gap-8 px-4 transition-transform duration-[800ms] ease-[cubic-bezier(0.22,1,0.36,1)]"
                        style={{
                            transform: `translateX(calc(50% - ${currentIndex * (cardWidth + gap) + cardWidth / 2}px))`
                        }}
                    >
                        {services.map((service, index) => (
                            <ServiceCard
